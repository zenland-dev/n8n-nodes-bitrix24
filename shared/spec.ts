 ...
 *
 * Bitrix24's REST is RPC — a method name and a JSON body — so an operation is
 * little more than the parameters it shows and the call it makes. Keeping them as
 * data lets a node with hundreds of operations share one dispatcher, one error
...
export interface Operation {
	...
	value: string;
	name: string;
	/** Becomes the tool name an AI agent sees, e.g. "Create a deal". */
	action: string;
	 ...
	 */
	execute?(this: IExecuteFunctions, itemIndex: number): Promise<IDataObject | IDataObject[] | undefined>;
	/**
